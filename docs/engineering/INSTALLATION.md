...
This guide accompanies the **v0.4.1 preview**. Use the archive route only
...
Use this route when the [v0.4.1 release](https://github.com/SuperDuperDave/agent-relay/releases/tag/v0.4.1) lists both
`relay-0.4.1-linux-x86_64.tar.gz` and its `.sha256` file under Assets.
...
curl --fail --location --remote-name https://github.com/SuperDuperDave/agent-relay/releases/download/v0.4.1/relay-0.4.1-linux-x86_64.tar.gz
curl --fail --location --remote-name https://github.com/SuperDuperDave/agent-relay/releases/download/v0.4.1/relay-0.4.1-linux-x86_64.tar.gz.sha256
sha256sum --check relay-0.4.1-linux-x86_64.tar.gz.sha256
tar -tvzf relay-0.4.1-linux-x86_64.tar.gz
```

Review the listed files under the single `relay-0.4.1/` root, then extract and
check the individual file hashes:

```sh
tar -xzf relay-0.4.1-linux-x86_64.tar.gz
cd relay-0.4.1
...
/usr/bin/python3 -I -S -B src/relay_bootstrap.py build-release \
  --output dist/multithread-preview --version 0.4.1
```
